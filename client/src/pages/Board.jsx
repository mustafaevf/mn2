    ...
    useEffect(() => {
        ...
        const getStatus = async () => {
            const response = await axios.get(
                `http://localhost:8080/api/board/${uuid}/status`
            )
        ...
        const getFields = async () => {
            const response = await axios.get(
                `http://localhost:8080/api/board/fields`
            )
            ...
            <div className="users bg box">
                {users.length > 0 ? (
                    users.map((us) => {
                        const player =
                            players &&
                            players.find((player) => player.id === us.user.id)
                        return player ? (
                            <div
                                className={`user light-bg ${player.color}${
